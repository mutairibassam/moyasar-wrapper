...
- **Every invoice created at Moyasar carries `metadata: { platform_item_id: <item uuid>, platform_batch_id: <batch uuid> }`** — the durable reconciliation join key (spec §7.3). No schema change: those are the existing item/batch UUIDs.
